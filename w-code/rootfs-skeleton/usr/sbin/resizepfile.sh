...
case $PUPMODE in
	...
	*) /usr/lib/gtkdialog/box_ok "$(gettext 'Resize personal storage file')" error "<b>$(gettext "Sorry, PLinux is not currently using a personal persistent storage file.")</b>" " " "$(eval_gettext "If this is the first time that you booted PLinux, say from a live-CD, you are currently running totally in RAM and you will be asked to create a personal storage file when you end the session (shutdown the PC or reboot).")
$(eval_gettext "If you have installed PLinux to hard drive, or installed such that personal storage is an entire partition, then you will not have a \${DISTRO_FILE_PREFIX}save.2fs file either.")"
...
if [ -d $HOMELOCATION ] ; then
	exec /usr/lib/gtkdialog/box_ok "$(gettext 'Resize personal storage file')" info "<b>$(gettext "PLinux is currently using a savefolder. There is no need to resize it")</b>" " "
fi
