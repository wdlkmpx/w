...
This is a daemon that provides certain ACPI management functions. PLinux will still work without it.
...
sys_logger
----------
This runs the daemons 'syslogd' and 'klogd', which log kernel and application events (espcially error messages) to various log files, mostly to /var/log/messages. This can be disabled and PLinux will still work.

udev
----
Runs the daemons 'udevd' and 'pup_event_frontend_d'.
This is a mechanism that receives information about hardware events from the kernel, such as a USB pen drive being plugged in or removed. If you don't want automatic detection of hardware changes while PLinux is running, PLinux will still work and you will save quite a lot of CPU usage and resources -- worth considering this one on a very slow CPU.
