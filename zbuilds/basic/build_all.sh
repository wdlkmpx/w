...
if [ -n "$z0base_dir" -a -f ${z0base_dir}/build.conf ] ; then
	...
	mkdir -p local-repositories/pbuilds/0sources
	export z0sources=$(cd local-repositories/pbuilds/0sources ; pwd)
	mkdir -p pbuilds-out
	export MWD=$(cd pbuilds-out ; pwd)
	[ ! -f pbuilds-out/func ] && ln -s ../${0%/build_all.sh}/func pbuilds-out/
	[ ! -f pbuilds-out/build.conf ] && ln -s ../${0%/build_all.sh}/build.conf pbuilds-out/
	[ ! -f pbuilds-out/split.sh ] && ln -s ../${0%/build_all.sh}/split.sh pbuilds-out/
	[ ! -f pbuilds-out/check-status.sh ] && ln -s ../pbuilds/check-status.sh pbuilds-out/
	[ ! -f pbuilds-out/mk-tar.sh ] && ln -s ../pbuilds/mk-tar.sh pbuilds-out/
	. /etc/DISTRO_SPECS
...
usage() {
	...
	echo
	echo "Just run ${0##*/}"
	echo "All pbuilds in the queue will be run and hopefully built"
	echo
...
building $pkg"
	if [ -n "$z0base_dir" ] ; then
		...
		[ "`which git`" != "" ] && echo "building $pkg from pbuild commit $(cd ${z0base_dir}/pkgs ; git log -1 $pkg | grep commit | cut -f 2 -d ' ' | cut -c -7)" >> ${z0logs}/${pkg}build.log
		cd ${MWD}/pkgs/$pkg
	else
		echo -n > ${z0logs}/${pkg}build.log
		[ "`which git`" != "" ] && echo "building $pkg from pbuild commit $(cd pkgs ; git log -1 $pkg | grep commit | cut -f 2 -d ' ' | cut -c -7)" >> ${z0logs}/${pkg}build.log
		cd pkgs/$pkg
	fi
	sh ${pkg}.pbuild 2>&1 | tee -a ${z0logs}/${pkg}build.log
	cd -
