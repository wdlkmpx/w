...
sanity_check() {
	...
	[ ! -d ./w-code ] && echo Missing w-code && exit
	[ ! -d ./w-distro ] && echo Missing w-code && exit
	if [ -e $WORK_DIR ]; then
...
get_target_arch() {
	...
	get_selection "Please choose target arch: " TARGET_ARCH "$(ls w-distro)"
...
get_source_distro() {
	get_selection "Please select source distro: " SOURCE "$(ls w-distro/$TARGET_ARCH)"
	get_selection "Please select version: " VERSION "$(ls w-distro/$TARGET_ARCH/$SOURCE)"
	# echo $SOURCE
}
...
NLS_DIR=nls-holder
BASE_CODE_PATH="w-code/rootfs-skeleton"
BASE_ARCH_PATH="w-arch/\$TARGET_ARCH/target/rootfs-skeleton"
BOOT_FILES_PATH="w-arch/\$TARGET_ARCH/build/boot"
EXTRAPKG_PATH="w-code/rootfs-packages"

# loads DEFAULT_REPOS, WITH_APT_DB and other repository options
. $(pwd)/${WORK_DIR}/repo-url 

### for ISO builder ###
CORE_SFS=core.sfs   # if you change this, change %makesfs params in basesfs too
OUTPUT_DIR=iso        # if you change this, change %makesfs params in basesfs too
OUTPUT_ISO=plinux.iso
...
EOF
	...
	cp w-distro/${TARGET_ARCH}/${SOURCE}/${VERSION}/* $WORK_DIR

	#============================================
	W_OUT=$WORK_DIR
	
	sdirs='initrd-progs kernel-kit w-arch w-code'

	# as files/dirs could be removed in future woofs, need to wipe entire target dirs first...
	for d in $sdirs
	do
		if [ -d ${W_OUT}/${d} ] ; then
			echo "Deleting ${W_OUT}/${d}"
	...
	cp -ar w-code/rootfs-skeleton/usr/share/doc z_doc/usr/share/
	cp -arf w-arch/${TARGET_ARCH}/target/rootfs-skeleton/* w-code/rootfs-skeleton
	)
	cat > ${W_OUT}/z_doc/pinstall.sh << _EOF
#!/bin/sh
	. etc/DISTRO_SPECS

echo "Configuring PLinux Help page..."
...
_EOF
	chmod 755 ${W_OUT}/z_doc/pinstall.sh
	sync
	(
	cd ${W_OUT}
	...
	do
		rm -fv ${W_OUT}/w-code/rootfs-skeleton/${i}
	done
