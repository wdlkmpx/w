...
The init script has been called the heart of PLinux.
This is not because it implements a lot of PLinux stuff, but because this is where PLinux starts.
When init starts, the full-blown PLinux is still just a number of files on a storage device.
The job of init is to use those files to build a PLinux based in RAM and then hand over control to it.
...
vmlinuz:
   This is the Linux part of PLinux, usually referred to as the Linux kernel.
   The boot process has already loaded this into memory and started running it before init starts.

initrd.gz:
   This contains the PLinux files that form the RAM based filesystem that is in place when init runs.
   The init script is one of these files.

The PLinux sfs files, core...sfs, zdrv...sfs, fdrv...sfs, ydrv...sfs, adrv...sfs:
(Where ... is a particular PLinux name and version, e.g. zdrv_slacko64_6.9.5.sfs)

core...sfs:
   This is the main PLinux file, containing most, if not all, the software that is in the current PLinux.
...
zdrv...sfs:
   ...
   Without this file, PLinux will usually still boot, but some devices will either not work or not work properly.
...
ydrv...sfs:
   Notionally a patch file. It can be used to override the contents of core...sfs.
...
* A typical frugal install of PLinux is a directory containing the above files.
* So init begins by establishing the location of this directory,
by looking for the core...sfs file.
...
* If it cannot locate the core...sfs file, it abandons the boot by dropping out to
a console with several error messages on the screen.
* Having located the core...sfs it proceeds to create a layered file system
...
* It then appends the core...sfs to this stack.
...
* The first time you reboot or shutdown, PLinux asks if you want to save the session.
* If you save the session you will be guided through a process to create a save layer,
to which PLinux will then copy any changed files.
...
* This file can be accessed in a running PLinux as /initrd/tmp/bootinit.log
...
DISTRO_SPECS:
   This is a file in initrd.gz that is created by the PLinux builder.
   It contains definitions of various information about a particular PLinux.
   ...
   Very significant for init are DISTRO_CORESFS, DISTRO_ZDRVSFS, DISTRO_FDRVSFS, DISTRO_YDRVSFS,
      DISTRO_ADRVSFS, which define the default filenames for each of the PLinux sfs files.
...
psubdir=</path/to/install/directory>
   If the PLinux files are not in the root of a partition, but in a sub-directory, the path of this directory, relative to the partition root, must be specified with this parameter.
   e.g. If the sdb2 partition is mounted as /mnt/sdb2 and the PLinux files are in /mnt/sdb2/tahr64, then "psubdir=tahr64" or "psubdir=/tahr64" must be specified.
   This parameter can specify subdirectories at more that a single level, e.g. "psubdir=plinux/tahr64" or "psubdir=/plinux/tahr64".
   If a leading "/" is not provided, init will add it.
   This is the default path for locating any PLinux file and any partition.

------------------------------------------------------------
pupsfs=<partition> Specifies the core...sfs partition
...
psave=<partition>  Specifies the save layer  partition
   ...
   Specifying psave=<partition> can be quite useful in directing all save layers to a different partition.
       ...
       If you forget to plug in the appropriate device, PLinux will simply do a first boot,
...
----
pupsfs=<partition>:<path>/<filename> Specifies the core...sfs file.
...
psave=<partition>:<path>/<filename>  Specifies the save layer file.
   ...
   Where <filename> is just a filename,
       e.g. "pupsfs=sdb2:/path/to/my-improved-core.sfs" or "psave=sdc2:my-improved-savefolder"
   ...
   For the purposes of the "psave=" specification a savefolder is considered to be just a file.
       ...
       Whereas "psave=sdb4:/lxpupsc/mysave/" says that the savefolder is on the sdb4 partition
           in the "/lxpupsc/mysave" directory, with the default savefolder name for the PLinux.

   It is not necessary to specify all elements,
       but if there is no ":" it is assumed to be a <partition> specification.
       e.g. "pupsfs=sdb2", specifies that the core...sfs file is on sdb2 in the default path with the default filename.
       "fdrv=:alternate-firmware.sfs", specifies that it's a file called alternate-firmware.sfs
       on the default partition in the default directory i.e. where the core...sfs is located.

   It is recommended that a pupsfs=<partition> always be specified.
   This enables init to go straight to that partition to find the PLinux files
       instead of searching through all partitions looking for core...sfs.

   ex: psave=sdc1:/path/to/tahrsave.4fs
   ex: psave=sdc1:tahrsave.4fs
   ex: zdrv=sdc1:/zz/myzz.sfs
   ex: adrv=sdd6:/plinux/drvs/custom/adrv.sfs
   ex: pupsfs=sdb2:/plinux/precise/core_precise_5.7.1.sfs
...
SAVEMARK
   ...
   device. It contains a single number. If the core...sfs is located in sdb2 and SAVEMARK
